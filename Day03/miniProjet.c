    ...
    do{
        ...
        if(reponse == 'O' || reponse == 'o'){
            ...
            if(!strchr(contacts[nbrContacts].nom, '\n'))
                    while(getchar()!='\n');
            do{
            	printf("Entrez le num�ro de t�l�phone du contact: ");
...
int modifierContact(size_t nbrContacts, contact contacts[]){
    ...
    fgets(nomContact, longueurChaineMax, stdin);
    do{
        printf("Entrez le nouveau num�ro de contact: ");
        scanf("%s", nouveauNum);
//        while(getchar()!='\n');
    ...
    for(i=0;i<nContacts;i++){
        if(memeLongueur(nomContact, contacts[i].nom)){
            ...
            if(nomContact[j]=='\0'){
                ...
                printf("** Le contact %s dont le num�ro est: %s, l'adresse email est: %s a �t� supprim� avec succ�s.\n", temp.nom, temp.numTelephone, temp.adresseEmail);
...
void main(){
    ...
    do{
        ...
        switch(choix){
            ...
            default:
                dernierChoix = 6;
                printf("Choix invalide. Veuillez r�ssayer.\n");
        }
    }while(choix!=6);
}
